...
class ConfigBuilder {
    ...
    private func buildTls(_ params: [String: String], vmessJson: [String: Any]? = nil) -> [String: Any]? {
        ...
        if let f = fp, !f.isEmpty {
            tls["utls"] = ["enabled": true, "fingerprint": f]
        }
    ...
    private func parseShadowsocks(_ link: String) -> [String: Any]? {
        ...
        if cleanLink.contains("@") {
            ...
            let portStr = parts[1].index(after: lastColon) < parts[1].endIndex ? String(parts[1][parts[1].index(after: lastColon)...]) : ""
            port = Int(portStr) ?? 0
        } else {
    ...
    private func parseWireGuard(_ link: String) -> [String: Any]? {
        ...
        if let addr = params["address"], !addr.isEmpty {
            outbound["local_address"] = addr.components(separatedBy: ",").map { $0.trimmingCharacters(in: .whitespaces) }
        }
    ...
    private func parseSsh(_ link: String) -> [String: Any]? {
        ...
        var outbound: [String: Any] = [
            "type": "ssh", "tag": "proxy",
            "server": host, "server_port": port, "user": user
        ]
        ...
        if let hk = params["hk"], !hk.isEmpty {
            outbound["host_key"] = hk.components(separatedBy: ",").map { $0.trimmingCharacters(in: .whitespaces) }
        }
        return outbound
    }
    ...
    private func buildFullConfig(outbound: [String: Any]) -> [String: Any] {
        ...
        let mtu = configOptions["mtu"] as? Int ?? 9000
        let mixedPort = configOptions["mixed-port"] as? Int ?? 2080

        var config: [String: Any] = [
            "log": ["level": logLevel, "timestamp": true],
            "dns": buildDns(),
            "inbounds": buildInbounds(mtu: mtu, mixedPort: mixedPort),
    ...
    private func buildInbounds(mtu: Int, mixedPort: Int) -> [[String: Any]] {
        return [
            [
                "type": "tun", "tag": "tun-in",
                "interface_name": "utun",
    ...
    private func buildExperimental() -> [String: Any] {
        ...
        if enableClashApi {
            exp["clash_api"] = ["external_controller": "127.0.0.1:\(clashApiPort)"]
        }
        return exp
    }
